...
export function computeZodiacDetailedBreakdown(dfUpToT: HistoryRecord[], settings: IndicatorSettings): ZodiacBreakdown[] {
  const scores: Record<string, number> = {};
  const factorDetails: Record<string, Record<string, { score: number; desc: string }>> = {};
  const allZodiacs = Object.keys(ZODIAC_MAPPING);
  ...
  const total = dfUpToT.length;
  if (total < 20) {
    return allZodiacs.map(z => ({
      zodiac: z,
      totalScore: 50.0,
      factors: [{ name: "系统冷启动", score: 50.0, desc: "历史数据少于20期，赋予基础平权分50.0" }]
    }));
  }
  ...
  const addFactor = (z: string, factorName: string, val: number, desc: string) => {
    if (val === 0) return;
    scores[z] = (scores[z] || 0) + val;
    factorDetails[z][factorName] = { score: Number(val.toFixed(2)), desc };
  };
  ...
  if (indicators.ENABLE_HISTORICAL_HEAT) {
    ...
    allZodiacs.forEach(z => {
      const freq = (zCounts[z] || 0) / total;
      const val = freq * 100 * w;
      addFactor(z, "历史热度", val, `历史出现频率 ${(freq * 100).toFixed(1)}% (权重 ${w}x)`);
    });
  }

  const spans = [
    { span: 10, ind: "ENABLE_RECENT_HEAT_10", wKey: "RECENT_HEAT_10_WEIGHT", name: "近期热度(10期)" },
    { span: 20, ind: "ENABLE_RECENT_HEAT_20", wKey: "RECENT_HEAT_20_WEIGHT", name: "近期热度(20期)" },
    { span: 50, ind: "ENABLE_RECENT_HEAT_50", wKey: "RECENT_HEAT_50_WEIGHT", name: "近期热度(50期)" }
  ];

  spans.forEach(({ span, ind, wKey, name }) => {
    if (indicators[ind] && total >= span) {
      const w = weights[wKey] ?? 1.0;
      const recentSlice = dfRich.slice(-span);
      const recentCounts: Record<string, number> = {};
      ...
      allZodiacs.forEach(z => {
        const freq = recentCounts[z] / span;
        const val = freq * 100 * w;
        addFactor(z, name, val, `近 ${span} 期内出现 ${recentCounts[z]} 次，频率 ${(freq * 100).toFixed(1)}%`);
      });
    }
  });
  ...
  if (indicators.ENABLE_MISSING_VALUE) {
    ...
    allZodiacs.forEach(z => {
      ...
      const val = ratio * 10 * w;
      addFactor(z, "遗漏值因子", val, `当前遗漏 ${m.currentMissing} 期 / 平均间隔 ${m.avgInterval.toFixed(1)} 期 (比率 ${ratio.toFixed(2)})`);
    });
  }
  ...
  if (indicators.ENABLE_AVERAGE_INTERVAL) {
    ...
    allZodiacs.forEach(z => {
      const m = missingStats[z];
      const val = (12.0 / m.avgInterval) * 10 * w;
      addFactor(z, "平均间隔", val, `历史平均开出间隔 ${m.avgInterval.toFixed(1)} 期`);
    });
  }
  ...
  if (indicators.ENABLE_COLD_HOT_BALANCE) {
    ...
    allZodiacs.forEach(z => {
      ...
      const histFreq = (zCounts[z] || 0) / total;
      const momentum = Math.max(0.0, recentFreq - histFreq);
      const val = momentum * 50 * w;
      addFactor(z, "冷热动能平衡", val, `近15期热度 (${(recentFreq * 100).toFixed(1)}%) 超越历史基准`);
    });
  }
  ...
  if (indicators.ENABLE_MARKOV && total > 2) {
    ...
    allZodiacs.forEach(z => {
      const prob = totalTrans > 0 ? (lastZTrans[z] || 0) / totalTrans : (1.0 / 12.0);
      const val = prob * 100 * w;
      addFactor(z, "马尔可夫转移链", val, `上期开出‘${lastZ}’后转移至‘${z}’的历史概率 ${(prob * 100).toFixed(1)}%`);
    });
  }
  ...
  if (indicators.ENABLE_WAVE_REVERSION) {
    ...
    const waveCounts = { "红": 0, "蓝": 0, "绿": 0 };
    recent15.forEach(r => {
      if (r.waveColor && r.waveColor in waveCounts) waveCounts[r.waveColor]++;
    });
    const recentWavePct = {
      "红": waveCounts["红"] / 15,
      "蓝": waveCounts["蓝"] / 15,
    ...
    allZodiacs.forEach(z => {
      ...
      const zBiasScore = sumBias / nums.length;
      const val = zBiasScore * 100 * w;
      addFactor(z, "波色纠偏", val, `包含波色近期偏离均值之自我校正补偿`);
    });
  }
  ...
  if (indicators.ENABLE_ODD_EVEN_REVERSION) {
    ...
    allZodiacs.forEach(z => {
      ...
      const evens = 1 - odds;
      const zOeBias = (odds * biasOdd) + (evens * biasEven);
      const val = zOeBias * 100 * w;
      addFactor(z, "单双纠偏", val, `单双比例短期失衡修正`);
    });
  }
  ...
  if (indicators.ENABLE_SIZE_REVERSION) {
    ...
    allZodiacs.forEach(z => {
      ...
      const smalls = 1 - bigs;
      const zSzBias = (bigs * biasBig) + (smalls * biasSmall);
      const val = zSzBias * 100 * w;
      addFactor(z, "大小纠偏", val, `大小比例短期失衡修正`);
    });
  }
  ...
  if (indicators.ENABLE_TAIL_REVERSION) {
    ...
    allZodiacs.forEach(z => {
      ...
      const zTailBias = sumTailBias / nums.length;
      const val = zTailBias * 150 * w;
      addFactor(z, "尾数纠偏", val, `尾数冷热分布回归纠偏`);
    });
  }
  ...
  if (indicators.ENABLE_CONSECUTIVE_PENALTY) {
    ...
    allZodiacs.forEach(z => {
      if (z === lastZ) {
        const val = 10.0 * Math.abs(w) * (w / Math.abs(w));
        addFactor(z, "连续出现惩罚", val, `上期刚开出，触发连庄惩罚扣分`);
      }
      if (lastZ2 && lastZ === lastZ2 && z === lastZ) {
        const val = 20.0 * Math.abs(w) * (w / Math.abs(w));
        addFactor(z, "连续出现惩罚", val, `连续两期开出，触发强惩罚`);
      }
    });
  }
  ...
  if (indicators.ENABLE_MAX_MISSING_RECOVERY) {
    ...
    allZodiacs.forEach(z => {
      ...
      if (m.currentMissing >= m.maxMissing * 0.8) {
        const recoveryFactor = Math.pow(m.currentMissing / m.maxMissing, 2);
        const val = recoveryFactor * 15 * w;
        addFactor(z, "极限遗漏回补", val, `当前遗漏逼近历史最大遗漏 (${m.currentMissing}/${m.maxMissing})，触发爆发回补`);
      }
    });
  }
  ...
  if (indicators.ENABLE_CYCLE_ANALYSIS && total > 30) {
    ...
    allZodiacs.forEach(z => {
      ...
      const m = missingStats[z];
      if (bestLag > 0 && m.currentMissing === bestLag && maxCorr > 0.05) {
        const val = maxCorr * 50 * w;
        addFactor(z, "周期自相关谱", val, `契合自相关滞后周期 ${bestLag} 期，相关系数 ${maxCorr.toFixed(2)}`);
      }
    });
  }
  ...
  if (indicators.ENABLE_SIMILAR_WINDOW && total > 10) {
    ...
    const zSeries = dfRich.map(r => r.zodiac);
    const patternLen = 2;
    const targetPattern = zSeries.slice(-patternLen);
    const successorCounts: Record<string, number> = {};
    allZodiacs.forEach(z => { successorCounts[z] = 0; });
    let matches = 0;

    for (let i = 0; i < zSeries.length - patternLen; i++) {
      if (zSeries[i] === targetPattern[0] && zSeries[i + 1] === targetPattern[1]) {
        const succ = zSeries[i + 2];
        if (succ && successorCounts[succ] !== undefined) {
          successorCounts[succ]++;
          matches++;
        }
      }
    }
    ...
    allZodiacs.forEach(z => {
      const cnt = successorCounts[z] || 0;
      const prob = matches > 0 ? cnt / matches : (1.0 / 12.0);
      const val = prob * 80 * w;
      if (val !== 0) {
        addFactor(z, "相似历史窗口", val, `历史相似走势匹配到 ${matches} 次，后继出现概率 ${(prob * 100).toFixed(1)}%`);
      }
    });
  }
  ...
  return allZodiacs.map(z => {
    const raw = scores[z] || 0;
    const normalized = 10 + ((raw - minS) / spanS) * 85;
    const factorMap = factorDetails[z] || {};
    const factorsList = Object.entries(factorMap).map(([name, data]) => ({
      name,
      score: data.score,
      desc: data.desc
    ...
    return {
      zodiac: z,
      totalScore: Number(normalized.toFixed(1)),
      factors: factorsList
    };
